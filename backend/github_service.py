...
GH_BOT_SYSTEM = """You are repoDoc, an autonomous AI bug-fixing agent. You raised this PR by:
...
class GitHubService:
    ...
    async def list_branches(self, owner: str, repo: str) -> list:
        """Return list of {name, sha} for all branches (paginated up to 100)."""
        data = await self._get(f"/repos/{owner}/{repo}/branches?per_page=100")
    ...
    async def get_commit(self, owner: str, repo: str, sha: str) -> dict:
        """Get full commit metadata including files (with patches)."""
        return await self._get(f"/repos/{owner}/{repo}/commits/{sha}")

    def build_diff_from_commit(self, commit_data: dict) -> str:
        """Build a unified diff string from GitHub commit API response."""
    ...
    async def create_issue(self, owner: str, repo: str, title: str, body: str, labels: list = None) -> str:
        """Create a GitHub issue. Returns html_url or empty string on failure."""
    ...
    async def create_pr(self, repo_url: str, repo_dir: str, analysis_id: str, fixes: list,
                        base_branch: str = None) -> str:
        owner, repo = self._parse_repo(repo_url)
        short_id = analysis_id[:8]

        # Determine base branch (default if not provided)
        if not base_branch:
            base_branch = await self.get_default_branch(owner, repo)

        # Branch name encodes the originating branch so multiple watchers don't collide
        safe_base = re.sub(r"[^a-zA-Z0-9_-]", "-", base_branch)[:30]
        branch_name = f"repodoc/fix-{safe_base}-{short_id}"
        ...
        pr_body = f"""## repoDoc Autonomous Fix

This PR was automatically generated by **[repoDoc](https://repodoctor-1.preview.emergentagent.com)** — an autonomous bug fixing agent.
...
"""
        ...
        pr_data = await self._post(
            ...
            {
                "title": f"fix: {len(verified_fixes)} bug(s) fixed by repoDoc [{short_id}]",
                "body": pr_body,
                "head": branch_name,
                "base": base_branch,
            },
    ...
    async def _check_and_reply(self, analysis: dict, db, llm_key: str):
        ...
        try:
            ...
            for comment in comments:
                ...
                if "*— repoDoc AI*" in body or body.startswith("> ") and "repoDoc AI" in body:
